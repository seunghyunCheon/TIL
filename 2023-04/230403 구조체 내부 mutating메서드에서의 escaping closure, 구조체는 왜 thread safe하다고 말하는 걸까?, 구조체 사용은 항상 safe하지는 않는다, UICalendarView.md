...
그래서 대안으로 [self] in을 사용할 수도 있는데 이는 클로저의 생성시점에 `self`를 복사해놓는 것이기 때문에 생성시점의 `self`만 사용하기 때문에 컴파일에러가 보이지 않는다. 하지만 실제 인스턴스의 프로퍼티가 변경되더라도 클로저에서 캡처하고 있는 `self`는 생성시점만 캡처하고 있기 때문에 이 `self`의 프로퍼티는 변하지 않는다.
즉 escaping closure를 통해 다른 컨텍스트에서 실행중인 `self`내의 프로퍼티는 고정된 값이기 때문에 현재`self`의 프로퍼티가 변경되더라도 반영할 수 없다. 그래서 이 상황에서 클래스를 사용해 프로퍼티의 변화를 추적했다. 
